...
(defn get-by-ids
  [db table ids {:keys [fields order-key order where order? id-key]
                 :or {fields [:*]
                      ...
                      order? true}}]
  (if (seq ids)
    (when-let [result (seq (query db (cond->
                                   {:from [table]
                                    :select fields
                                    :where (if where
                                   ...
                                   (assoc :order-by (if (not= order-key :flake_id)
                                                      ...
                                                      [[order-key order]])))))]
      (let [result (su/normalize result)]
        (some->>
         (for [id ids]
           (clojure.core/get result id))
         (remove nil?))))))
...
(defn wrap-cursor [honey-map cursor]
  (let [{:keys [after before order limit order-key]
        ...
        order-by (cond
                   (coll? order-key)
                   (mapv (fn [k] [k order]) (conj (vec order-key) :flake_id))

                   (not= order-key :flake_id)
                   (if (contains? #{:last_reply_at} order-key)
                     [[order-key order :nulls-last] [:flake_id order]]
                     [[order-key order] [:flake_id order]])
