...
(defn make-request [method end-point positional query]
  (let [{:keys [auth throw-exceptions follow-redirects accept
        ...
        req (if (#{:post :put :delete :patch} method)
              (cond-> (assoc req :body (json/generate-string (or raw-query proper-query)))
                (and client-id client-secret)
                (assoc :query-params {"client_id" client-id
...
(defn base64-encode
  [content]
  (.encode (Base64/getEncoder) (.getBytes content)))

(defn base64-decode
  [bytes]
  (String. (.decode (Base64/getDecoder) bytes)))
